...
function input_new_recipe($category, $asal, $judulResep, $deskripsi, $lamaPersiapan, $lamaMasak, $bahan, $peralatan, $langkah, $id_user)
{
...
function delete_recipe($id_recipe)
{
    ...
    if (!empty($id_recipe)) {
        ...
        echo
        "<script>alert('Data Berhasil Dihapus');location='../profil.php';</script>";
    } else {
        echo "<script>alert('Error');window.history.go(-1);</script>";
    }
}
